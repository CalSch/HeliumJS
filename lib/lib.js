...
const helium = {
	...
	Spritesheet: function (src,rows,cols) {
		let img=new Image();
		img.src=src;
		let self = {
			src: src, rows: rows, cols: cols, image: img,
			width: img.width,
			height: img.height};
		self = {
			src: src, rows: rows, cols: cols, image: img,
			width: img.width,
			height: img.height,
			colSize: self.height / self.cols,
			rowSize: self.width / self.rows,
			sprites: self.rows * self.cols,
			getSpriteRect: function (x, y) {
				return {
					x: x * self.rowSize,
					y: y * self.colSize,
					w: self.rowSize,
	...
	ImageFromSS: function (x,y,sprX,sprY,ss) {
		let self = {
			x,sprX,
			y,sprY,
			ss,
			...
			draw: function(pos,ctx) {
				let rect=self.ss.getSpriteRect(self.sprX,self.sprY);
				pos=helium.util.getRectCorner(pos.x,pos.y,rect.w,rect.h)
				ctx.drawImage(self.ss.image,
					rect.x,
					rect.y,
					rect.w,
					rect.h,
	...
	Animation: function (spritesheet,frameRate,x,y){
		let self={
			x,y,frameRate,
			sprX:0,
			sprY:0,
			t: 0,
			draw: function(pos,ctx) {
				let rect=spritesheet.getSpriteRect(self.sprX,self.sprY);
				//console.log(rect)
				ctx.drawImage(spritesheet.image,
					...
					rect.w,
					rect.h,
					helium.util.getRectCorner(self.x,self.y,rect.w,rect.h).x,
					helium.util.getRectCorner(self.x,self.y,rect.w,rect.h).y,
					rect.w,
					rect.h
				)
			},
			update: function (dt) {
				self.t = (self.t || 0)+dt;
				if (self.t>1000/self.frameRate) {
					self.sprX++;
					if (self.sprX>=spritesheet.cols) {
						self.sprX=0;
						self.sprY++;
						if (self.sprY>=spritesheet.rows) {
							self.sprY=0;
						}
					}
					self.t=0;
